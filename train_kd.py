"""Distill the RGB-only KD student from a trained single-dataset IFD teacher run.

Logit-based KD: loss = (1 - kd_alpha) * CE(student, label) + kd_alpha * T^2 * KL(student/T || teacher/T).
"""
import os
import argparse
...
from common import build_transforms, build_dataset, build_model, save_json, load_json, set_data_root
from utils import collate_fn, create_log_directory
from engine import run_training_loop


def parse_args():
    p = argparse.ArgumentParser(description="Train the RGB-only KD student from an IFD teacher run")
    p.add_argument("--teacher_log_dir", type=str, required=True,
                    help="Path to a completed train.py run (must contain config.json and checkpoints/best_model.pth)")
...
def main():
    ...
    log_dir = create_log_directory(base_dir=os.path.join(args.log_base_dir, dataset, "imgflowdepth_kd"))
    print(f"[KD] Created log directory: {log_dir}")

    transform = build_transforms(img_size, flow_channels, flow_representation)
    ds_kwargs = dict(img_size=img_size, num_frames=num_frames, flow_channels=flow_channels,
                      flow_representation=flow_representation, flow_offset=flow_offset,
                      protocol=protocol, n_split=n_split, transform=transform)
    train_ds = build_dataset(dataset, "train", **ds_kwargs)
    val_ds = build_dataset(dataset, "devel", **ds_kwargs)
    print(f"[KD] Dataset: {dataset} | Train: {len(train_ds)} | Val: {len(val_ds)}")
    ...
    if args.eval:
        print("\n[Eval] Running evaluate.py on the KD student run...")
